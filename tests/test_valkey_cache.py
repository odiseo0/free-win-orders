...
async def test_valkey_cache_applies_namespace_and_ttl() -> None:
    ...
    await cache.start()
    await cache.set("orders:item:1", "payload", ttl_seconds=300)

    assert client.pinged is True
    assert client.data["free-win:test:orders:item:1"] == "payload"
    assert client.expirations["free-win:test:orders:item:1"] == 300
    assert await cache.get("orders:item:1") == "payload"

    await cache.delete("orders:item:1")
...
async def test_valkey_cache_decodes_byte_responses() -> None:
    client = FakeValkeyClient()
    client.data["free-win:orders:item:1"] = b"payload"
...
async def test_valkey_cache_deletes_prefix_with_scan_batches() -> None:
    client = FakeValkeyClient()
    client.data = {
        **{f"free-win:orders:list:{index}": "payload" for index in range(101)},
        "free-win:orders:item:1": "keep",
    }
    cache = ValkeyCache(client)

    await cache.delete_prefix("orders:list:")

    assert [len(batch) for batch in client.delete_batches] == [100, 1]
    assert client.data == {"free-win:orders:item:1": "keep"}
