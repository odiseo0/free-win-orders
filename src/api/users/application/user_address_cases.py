...
from src.api.users.domain import UserAddressCreate, UserAddressUpdate
...
) -> Ok[UserAddress] | Err[EmptyType]:
    data = await dao.get(db, user_address_id)
...
) -> Ok[tuple[list[UserAddress], int]] | Err[list]:
...
async def update(
    ...
    result = await dao.update(db, user_address_id, obj_in)
...
async def remove(db: AsyncSession, user_address_id: int) -> Ok[UserAddress] | Err[None]:
    user_address = await dao.get(db, user_address_id)

    if user_address is EmptyType:
        return Err("Error")

    result = await dao.delete(db, db_object=user_address)
